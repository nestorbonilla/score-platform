...
const EcoDialog: React.FC<EcoDialogProps> = ({ onClose }) => {
  const [isProgramConfirmed, setIsProgramConfirmed] = useState(false);
    ...
    const { loading, error, data } = useQuery(GET_ATTESTATION, {
        ...
        onCompleted: (data) => {
            console.log('Eco => Query completed. Data:', data);
        },
        onError: (error) => {
            console.error('Eco => GraphQL query error:', error);
        }
    });
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const schemaEncoder = new SchemaEncoder(process.env.NEXT_PUBLIC_ECO_SCHEMA!);
            const schemaUID = process.env.NEXT_PUBLIC_ECO_SCHEMA_ID;
            
            const encodedData = schemaEncoder.encodeData([
                { name: 'programReceived', value: true, type: 'bool' },
                { name: 'programName', value: 'Eco Tech', type: 'string' },
            ]);
    ...
    return (
        ...
        >
            {isAttesting ? (
                ...
                <div>
                    <DialogHeader>
                      <DialogTitle>Eco Tech</DialogTitle>
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="items-center gap-4">
                            <Label htmlFor="address">
                                Create an attestation confirming you have succesfully attended a Eco Tech educational program.
